...
import nl.ssischaefer.savaragerow.v1.query.ddl.RenameColumnQuery;
import nl.ssischaefer.savaragerow.v1.query.TableService;
import nl.ssischaefer.savaragerow.v1.util.RequestParams;
import nl.ssischaefer.savaragerow.v1.util.sql.SQLiteDatatype;
...
public class SchemaController {
    ...
    public static final Route renameColumn = (Request request, Response response) -> {
        ...
        new RenameColumnQuery().setTable(table).setFrom(column).setTo(columnSchema.getName()).execute();
        return "";
    };
    ...
    public static final Route addTable = (Request request, Response response) -> {
        ...
        new CreateTableQuery().setTableName(table).setColumns(Collections.singletonList(columnSchema)).execute();
        return "";
    };
}
